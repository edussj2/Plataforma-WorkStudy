<?php 
	...
	class subcategoriaControlador extends subcategoriaModelo
	{
		...
		public function agregar_subcategoria_controlador(){
			...
			$descripcion = mainModel::limpiar_cadena($_POST['subcategoria_nombre_reg']);
			$icono = mainModel::limpiar_cadena($_POST['subcategoria_icono_reg']);
			$cat = mainModel::limpiar_cadena($_POST['subcategoria_categoria_reg']);
            $vigencia =  mainModel::limpiar_cadena($_POST['subcategoria_estado_reg']);


            /*--VALIDACIONES--*/
            if($descripcion== "" || $cat =="Sin Registro"){

                $alerta = ["Alerta"=>"simple", "Titulo"=>"Ocurrió un error","Texto"=>"Complete los campos requeridos","Tipo"=>"warning"];

            }else{

                    $consulta1 = mainModel::ejecutar_consulta_simple("SELECT SubDescripcion	 FROM subcategoria WHERE SubDescripcion	='$descripcion'");
                    ...
                    if($consulta1->rowCount()>=1){
                            ...
                            if(strlen( $descripcion ) > 50){
                                    ...
                                    if($vigencia != "Vigente" && $vigencia != "Sin Vigencia" ){
                                        ...
                                        $guardarsubcategoria = subcategoriaModelo::agregar_subcategoria_modelo($datossubcategoria);

                                        if($guardarsubcategoria->rowCount()>=1){
                                                $alerta = ["Alerta"=>"limpiar", "Titulo"=>"SUBCATEGORÍA REGISTRADA","Texto"=>"Los datos se registrarón con éxito.","Tipo"=>"success"];
                                        }else{
                                                $alerta = ["Alerta"=>"simple", "Titulo"=>"Ocurrió un error","Texto"=>"Hubo un problema con el registro de la subcategoría, intente nuevamente","Tipo"=>"error"];
                                        }
        ...
        public function paginador_subcategoria_controlador($pagina,$registros,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
                $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM subcategoria INNER JOIN categoriacurso ON subcategoria.idCatCurso = categoriacurso.idCategoriaCurso WHERE SubDescripcion LIKE '%$busqueda%' ORDER BY SubDescripcion ASC LIMIT $inicio,$registros";

                $paginaURL = "subcategoriaBuscar";
            }else{
                $consulta = "SELECT SQL_CALC_FOUND_ROWS * FROM subcategoria INNER JOIN categoriacurso ON subcategoria.idCatCurso = categoriacurso.idCategoriaCurso ORDER BY SubDescripcion ASC LIMIT $inicio,$registros";

                $paginaURL = "subcategoriaLista";
            }
            ...
            if($total>=1 && $pagina <= $Npaginas){
                ...
                foreach ($datos as $rows) {
                    $tabla.= '<tr>
                                ...
                                <td>
                                    '.$rows['CatCursoDescripcion'].'
                                </td>
                                <td>
                                    <div class="identificacion">
                                        <h6><i class="'.$rows['SubIcono'].'"></i> &nbsp; '.$rows['SubDescripcion'].'</h6>
                                    </div>
                                </td>
                                <td>
                                    '.$rows['SubVigencia'].'
                                ...
                                <td>
                                    <a href="'.SERVERURL.'subcategoriaEditar/'.mainModel::encryption($rows['idSubcategoria']).'/" class="btn btn-success" data-toggle="tooltip" title="Actualizar Cuenta" data-placement="bottom">
                                        <i class="fas fa-sync"></i>
                                    </a>
                                </td>
                                <td>
                                    <form action="'.SERVERURL.'ajax/subcategoriaAjax.php" method="POST" class="FormularioAjax" data-form="delete" entype="multipart/form-data">
                                        <input type="hidden" name="subcategoria_id_del" value="'.mainModel::encryption($rows['idSubcategoria']).'">
        ...
        public function eliminar_subcategoria_controlador(){
            ...
            $EliminarSector = subcategoriaModelo::eliminar_subcategoria_modelo($id);
        ...
        /*DATOS*/
        public function datos_subcategoria_controlador($tipo,$codigo){

            $codigo = mainModel::decryption($codigo);
            $tipo = mainModel::limpiar_cadena($tipo);

            return subcategoriaModelo::datos_subcategoria_modelo($tipo, $codigo);
        }
